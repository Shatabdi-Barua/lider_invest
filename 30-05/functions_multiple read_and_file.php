...
// 	$table_name = $wpdb->prefix . 'arogga_data';
...
// 		medicine_weight text NOT NULL,
// 		medicine_type text NOT NULL,
// 		medicine_company text NOT NULL,
//     people_view text NOT NULL,
//     generic text NOT NULL,
//     best_price_amount text NOT NULL,
//     previous_price_amount text NOT NULL,
//     cart text NOT NULL,
//     variation_list text NOT NULL,
...
// function read_csv_only_once() {
//   if ( did_action( 'init' ) >= 6 )
//       return; 

//   if( ! get_option('run_read_csv_once') ) {

//       read_csv(); // Run the function

//       update_option( 'run_read_csv_once', true );
//   }
// }
function read_csv(){
  // $myfile = fopen("/home3/bdmonst1/public_html/wp/lider-invest/wp-content/themes/liderinvest/newfile.txt", "a") or die("Unable to open file!");
  // $txt = 'read_csv_function->'."\n";
  // fwrite($myfile, $txt);
  // fclose($myfile);
  ...
  $csv_url = get_template_directory_uri().'/css/arogga_new_5.csv';
  if(!empty($csv_url)){
    ...
    $flag = 0;
    $data_count = count($col_body);
   
      for ($x=0; $x<$data_count; $x++)
      { 
        foreach ($p_id as $id) {
          $product = wc_get_product( $id );
          $prev_sku = $product->get_sku();
        // echo 'db sku: '.$prev_sku;
        // echo '<br> col sku : '. $col_body[$x]['sku'];
        if($prev_sku == $col_body[$x]['sku']){
          $prev_prdct_id = $id;
      ...
      if($flag ==1)
      {
        $prev_id_array[$x] = $prev_prdct_id;
      }
      else{
        $prev_id_array[$x] = $prev_id;
      }
     

      
      // echo '<br>previous id: '.$prev_prdct_id;
     
        // trz_at_add_variation_product($col_body[$x],$col_head, $trz_at_variations,$prev_prdct_id);
      
      
    //    }      
    }  
    // echo '<pre>';
    //   print_r($prev_id_array);
    // exit();
    // if($flag==0){  
    //   $prev_prdct_id ='';
    // }
    ...
    // }
    // echo 'csv<br>';
    trz_at_add_variation_product($col_body,$col_head, $trz_at_variations,$prev_id_array);
    fclose($file);
  }  
}
...
function add_vars($nam, $vals)
{
    ...
    // for ($ff=0; $ff < count($vals) ; $ff++) 
	  // {
      if( ! term_exists( $vals, $taxonomy ) )
		  { 
        $term_data = wp_insert_term($vals, $taxonomy ); 
        $term_id = $term_data['term_id']; 
		  } 
      else 
      { 
        $term_id = get_term_by( 'name', $vals, $taxonomy )->term_id; 
      } 
	// }
	return $term_id; 
}
...
function trz_at_get_variations($attrs,$col_head, $col_body)
{
  ...
  foreach ($attrs as $attr) {
    ...
    $trz_at_variations[$attr] = $temp_array;    
  }
  return $trz_at_variations;
}
//attribute and variations set
function trz_at_add_variation_product($data,$col_head, $trz_at_variations,$prev_id_array)
{  
  // echo '<pre>';
  // print_r($prev_id_array);
  // exit(); 
  ...
  foreach($prev_id_array as $key=>$prev_id)
  {
    ...
    if($prev_id !='')
    {
      // echo 'has id';
      // exit();
      // $product = wc_get_product($prev_id);
      ...
      // }
      $product_id = $prev_id;
      // $product = new WC_Product_Variable( $prev_id );
      // wp_update_post( array('ID' => $product_id, 'post_content' => $data['other_desciption']) );
      // $product->save();    
    ...
    else
    {
      ...
      // {
        ...
        $product_id = trz_at_add_product($data[$key]);
        // Get an instance of the WC_Product_Variable object and save it
        $product = new WC_Product_Variable( $product_id );
        $product->save();
        $product->set_sku($data[$key]['sku']);
        $product->set_description($data[$key]['other_desciption']);
...
// exit();
  ...
  //   $product->set_description($data['other_desciption']);
  ...
  {
    // print($pro_id_array[$tr]);
    // exit();
    $i = 0; 
    $product = new WC_Product_Variable( $pro_id_array[$tr] );
    $product_id = $pro_id_array[$tr];

    // $myfile = fopen("/home3/bdmonst1/public_html/wp/lider-invest/wp-content/themes/liderinvest/newfile.txt", "a") or die("Unable to open file!");
    // $txt = 'LINE 929->'.$product_id.'-->'.$tr."\n";
    // fwrite($myfile, $txt);
    // fclose($myfile);
    // echo '<pre>';
    ...
    {     
      ...
      //general attribute
        if($key == 'medicine_weight' || $key == 'medicine_type' || $key == 'medicine_company')
        {
          ...
          $option_string = '';
          $option_string = $option_string . $values[$tr] . ' | ';
          ...
          // }else{ //if not varriation
            ...
            $term = add_new_attribute($key, $values[$tr]); //29
            
            $at_val = $values[$tr];
            // exit();
            //     $at_val = implode(',',$values[$tr]);
                ...
                $taxonomy = 'pa_'.$key;    
                // echo '<br> pro_id: ';
                // print($product_id);echo '<br> value: ';
                // print(gettype($at_val));   echo '<br> taxonomy: ';
                // print(gettype($taxonomy));
      ...
      {
        $price_val = preg_replace('/৳/', "", $values[$tr]);
      ...
      {     
        ...
        $variation_val = str_ireplace('[','',$values[$tr]);
        ...
        else
        {
          ...
          if(count($vv))// Check Array Avaibality
          {                      
            $package_title = array();
            $get_first = $vv[0];
            ...
            if($has_star == false)
            {
              ...
              $atribute_array = [];
              for($p=0; $p<count($package_title); $p++)
              {
                foreach($package_title[$p]['attributes'] as $key=> $values)
                {
                  if($key== 'package_size')
                  {
      ...
      //         $destination = '/home3/bdmo'1/public_html/wp/lider-invest/wp-content/themes/liderinvest/images/arogga_image/'.$imageName;
...
function trz_at_add_product($data){
  ...
  // exit();
  $postname = sanitize_title( $data['medicine_name'] );
  $author = '1';

    $post_data = array(
        'post_author'   => $author,
        'post_name'     => $postname,
        'post_title'    => $data['medicine_name'],
        'post_content'  => '',
        'post_excerpt'  => '',
        'post_status'   => 'publish',
        'ping_status'   => 'closed',
...
function trz_at_create_product_variation( $product_id, $variation_data ){
  ...
  $v= $variation_data['attributes']['package_size'];
