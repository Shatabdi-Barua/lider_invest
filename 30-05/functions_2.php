...
function read_csv(){
  ...
  $csv_url = get_template_directory_uri().'/css/arogga_new_3.csv';
  if(!empty($csv_url)){
    ...
    $data_count = count($col_body);
      ...
      if($flag ==1)
      {
        $prev_id_array[] = $prev_prdct_id;
      }
      else{
        $prev_id_array[] = $prev_id;
      }
...
{  
  ...
  // exit(); 
  $pro_id_array = array();
  foreach($prev_id_array as $prev_id)
  {
    // echo 'prev id';
    if($prev_id !='')
    {
    ...
    else
    {
      ...
      for($i = 0; $i<count($data); $i++)
      {
        ...
        $product_id = trz_at_add_product($data[$i]);
        // Get an instance of the WC_Product_Variable object and save it
        $product = new WC_Product_Variable( $product_id );
        $product->save();
        $product->set_sku($data[$i]['sku']);
        $product->set_description($data[$i]['other_desciption']);
