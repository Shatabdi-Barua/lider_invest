...
function read_csv(){
  ...
  $csv_url = get_template_directory_uri().'/css/arogga_new_10.csv';
  if(!empty($csv_url)){
  ...
  for($tr = 0; $tr<count($data); $tr++)
  {
    ...
    {     
      ...
      elseif($key=='main_img')
      {
        ...
        // {           
              $upload_img = file_get_contents( $values[$tr]);
      ...
      elseif($key == 'product_images')
      {
            ...
            // {               
              ...
              if(substr($values[$tr], 0, 2)=== "['" && substr($values[$tr], -2, 2) === "']")
              {
                $pro_img = substr($values[$tr], 2, -2);
                $pro_img = explode(",",$pro_img); 
              }  
              // print_r( $pro_img);
              foreach($pro_img as $pimg)
              {
                // print($pimg);
                $upload_other_image = file_get_contents( $pimg);
