...
{  
  ...
  foreach ($trz_at_variations as $key => $values) 
  {
      ...
      else
      {
        ...
        {            
          ...
          if($has_star == false)
          {
            ...
            // $term_taxonomy_ids = wp_set_object_terms( $product->get_id(), 'pa_'.$key, true );
            // print_r($term_taxonomy_ids);
            // exit();
            // get the full term object
            // $term = get_term_by('id', $term_taxonomy_ids[0], 'pa_'.$key);
            // $term_id = get_term_by( 'name', $vals[$ff], $taxonomy )->term_id; 
            ...
            print_r($attr_tax);
            exit();
            $data_attr[$attr_index] = array(
              'name' => 'pa_'.$key, // set attribute name
              'value' => 'tabll', // set attribute value
              'is_visible' => 1,
              'is_variation' => 1,
              'is_taxonomy' => 1
            );  
            ...
            // $update_msg = update_post_meta( $product->get_id(),'_product_attributes', $theData);
            // print_r($update_msg);
            for ($i=0; $i<count($package_title); $i++)
            {
    ...
    elseif($key == 'medicine_weight' || $key == 'medicine_type' || $key == 'medicine_company')
    {
      // $term = add_new_attribute($key, $values); //29
      ...
      $taxonomy = 'pa_'.$key;       
      $term_taxonomy_ids = wp_set_object_terms($product_id, $at_val, $taxonomy, true);  //attribute value set            
