...
function read_csv(){
  ...
  $csv_url = get_template_directory_uri().'/css/arogga.csv';
  // print($csv_url);
  // exit();
  // $csv_url = 'arogga.csv';
//   echo getcwd();
  ...
  if($prev_prdct_id !=''){
    ...
    foreach ($trz_at_variations as $key => $values) {
      ...
      if(count($values)>1){ // if variation 
        ...
        //assign_attributes($key, $values, $product_id);
        // print_r($term);
        // print_r($term_slug);
        // exit();
        // wp_set_object_terms( $product->get_id(), '28', $term_slug, true);
        // print_r($product_id); //product_id = 5045
        // exit();
        // $term_taxonomy_ids = wp_set_object_terms($product_id, $values, $taxonomy, true);
        ...
        if($key == 'medicine_weight' || $key == 'medicine_type' || $key == 'medicine_company')
          {
            ...
            $term_taxonomy_ids = wp_set_object_terms($product_id, $at_val, $taxonomy, true);
                ...
                $data_attr[$i] = array(
                  ...
                  // print_r($data_attr);
        $i++;
                  // foreach($data_attr as $dt)
                  // {
                  //   // echo '<pre>';
                  //   // print_r($dt);
                  //   $new_arr = array($dt);
                  //   update_post_meta($product_id, '_product_attributes', array_merge($product_attributes, $new_arr));
                  // }
                  // //First getting the Post Meta
                    
                    // echo '<pre>';
                    // print_r($data_attr['medicine_type']);
                    // exit();
                  // //Updating the Post Meta
                    // update_post_meta($product_id, '_product_attributes', array_merge($product_attributes, $data_attr));
        ...
        // { 
          ...
          foreach ($value as  $v) {
            ...
            //   if(preg_match_all($pattern2, $col_key) !=0  ){
                ...
                //   $x = $x + $x * $y;
                  $trz_at_variation_data['regular_price']= $data_each['previous_price_amount'];
            ...
            //echo '<pre>';
            trz_at_create_product_variation( $product_id, $trz_at_variation_data );
            $product->save();
